...
import { LayoutDashboard, Users, MessageSquare, LogOut, Clock, ChevronRight, X } from 'lucide-react';
...
const AdminSidebar: React.FC<Props> = ({ activeView, onViewChange, onLogout, onClose, timeLeft, stats }) => {
  return (
    <aside className="w-64 shrink-0 bg-[#060E1A] border-r border-gold/10 flex flex-col h-full min-h-screen">
      {/* Logo + mobile close */}
      <div className="px-5 py-4 border-b border-gold/10 flex items-center justify-between gap-3">
        ...
        {/* X button — only shows on mobile */}
        {onClose && (
          <button
            onClick={onClose}
            className="lg:hidden text-white/50 hover:text-white p-1.5 rounded-md hover:bg-white/10 transition-colors shrink-0"
      ...
      <nav className="flex-1 px-3 py-5 space-y-1">
        ...
        {navItems.map(({ id, label, icon: Icon }) => {
          const badge =
            id === 'registrations' ? (stats ? stats.totalIndividual + stats.totalDelegation : null)
            : id === 'messages'    ? (stats?.totalContacts ?? null)
            : null;
          return (
            <button
              key={id}
              onClick={() => onViewChange(id)}
              className={`w-full flex items-center justify-between px-3 py-3 rounded-lg text-sm font-sans transition-all group ${
                activeView === id
              ...
              <div className="flex items-center gap-2">
                {badge !== null && (
                  <span className={`text-xs px-1.5 py-0.5 rounded-full ${
                    activeView === id ? 'bg-gold/20 text-gold' : 'bg-white/10 text-white/50'
                  }`}>
                    {badge}
                  </span>
                )}
